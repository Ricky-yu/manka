...
let timeoutClosure = {(endpoint: Endpoint, closure: MoyaProvider<MankaApi>.RequestResultClosure) -> Void in
...
let ApiProvider = MoyaProvider<MankaApi>(requestClosure: timeoutClosure)
let ApiLoadingProvider = MoyaProvider<MankaApi>(requestClosure: timeoutClosure, plugins: [LoadingPlugin])
...
extension MoyaProvider {
    ...
    open func request<T: HandyJSON>(_ target: Target,
        ...
        return request(target, completion: { (result) in
            ...
            guard let returnData = try? result.value?.mapModel(MankaResponseData<T>.self) else {
                completion(nil)
                return
            }
