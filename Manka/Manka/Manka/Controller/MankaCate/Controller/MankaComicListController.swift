//
//  MankaComicListController.swift
//  Manka
//
...
class MankaComicListController: MankaBaseController {
    ...
    private var comicList = [MankaComicModel]()
    ...
    private lazy var tableView: UITableView = {
        ...
        tableView.register(cellType: MankaComicTVCell.self)
    ...
    @objc private func loadData(more: Bool) {
        page = (more ? ( page + 1) : 1)
        ApiLoadingProvider.request(MankaApi.comicList(argCon: argCon, argName: argName ?? "", argValue: argValue, page: page),
                                   model: MankaComicListModel.self) { [weak self] (returnData) in
...
extension MankaComicListController: UITableViewDelegate, UITableViewDataSource {
